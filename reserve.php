		...
		<?php
			if(isset($_POST['submit_reservation']) && $_SERVER['REQUEST_METHOD'] === 'POST')
			{
                ...
                try
                {
                    ...
                    //Date Difference
                    $stmt_date = $con->prepare("select datediff(return_date, pickup_date) from reservations where `reservation_id`=(SELECT max(reservation_id) from reservations)");
					$stmt_date->execute();
					$diff_date = $stmt_date->fetch();

                    //Car Cost_per_day
                    $stmt_car_cost = $con->prepare("select cost_per_day from `reservations` join `cars` on car_id=id where reservation_id=(SELECT max(reservation_id)from reservations)");
                    $stmt_car_cost->execute();
                    $car_cost_per_day = $stmt_car_cost->fetch();

                    //Bill Calculation
                    $rent_bill = $diff_date[0] * $car_cost_per_day[0];
                    //Inserting Bill Details
                    $stmt_bill = $con->prepare("update `reservations` set bill = ? where `reservations`.`reservation_id` = (SELECT max(`reservations`.`reservation_id`) from `reservations`)");
                    $stmt_bill->execute(array($rent_bill));

                    //Car Name
                    $stmt_car = $con->prepare("select car_name from `reservations` join `cars` on car_id=id where reservation_id=(SELECT max(reservation_id)from reservations)");
                    $stmt_car->execute();
                    $car_name = $stmt_car->fetch();

                    // Invoice Display
					echo "<div class = 'alert alert-success'>";
                        echo "Great! Your reservation has been created successfully. "."<br>";
                        echo "Client Name: ".$full_name."<br>";
                        echo "Client Pn Number: ".$client_phonenumber."<br>";
                        echo "Car Name : ".$car_name[0]."<br>";
                        echo "Number of days Car reserved : ".$diff_date[0]."<br>";
                        echo "Rent cost : ".$rent_bill."<br>";
                        echo "Thank you for choosing our service <span>&#128522;</span><br> Invoice will be send to the Client's number. ";
                    echo "</div>";
											...
											foreach($available_cars as $car)
											{
												echo "<div class='itemListElement'>";
													echo "<div class = 'item_details'>";
														echo "<div>";
															echo $car['car_name'];
															echo "<div class='cost_details'>";
															    echo "Cost Per Day: ";
																echo $car['cost_per_day'];
															echo "</div>";
														echo "</div>";
